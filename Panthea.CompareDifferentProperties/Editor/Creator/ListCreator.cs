...
namespace Panthea.CompareDifferentProperties.Editor
{
    internal class ListCreator
    {
        ...
        {
            var genericType = a.GetUnderlyingType().Implement(typeof (IList<>)).GenericTypeArguments[0];
            ...
            else
            {
                ...
                sb.AppendLine($"foreach(var node2 in b.{a.Name})");
                sb.AppendLine("{");
                sb.Indeed++;
                sb.AppendLine("if(!IsEqual(node, node2))");
                sb.AppendLine("{");
                sb.Indeed++;
        ...
        public static void Check(InternalStringBuilder sb, MemberInfo a)
        {
            ...
            {
                GenerateCode.MethodName[genericType.TrueTypeString()] = () => ClassCreator.Check(sb, a);
                sb.AppendLine($"foreach(var node in a.{a.Name})");
                sb.AppendLine("{");
                sb.Indeed++;
